"""Tests for the MemoryStorage implementation."""
...
@pytest.mark.asyncio
async def test_append_and_get_events(storage: MemoryStorage) -> None:
    event = _make_event()
...
@pytest.mark.asyncio
async def test_get_events_returns_empty_for_unknown_jp(storage: MemoryStorage) -> None:
    result = await storage.get_events("nonexistent")
    assert result == []


@pytest.mark.asyncio
async def test_get_events_filter_by_event_type(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_events_filter_by_actor_id(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_events_filter_by_after(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_events_filter_by_before(storage: MemoryStorage) -> None:
    await storage.append_event(_make_event(event_id="e1", timestamp=EARLIER))
    await storage.append_event(_make_event(event_id="e2", timestamp=LATER))
...
@pytest.mark.asyncio
async def test_get_events_by_project(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_events_by_project_with_pagination(storage: MemoryStorage) -> None:
    for i in range(5):
        await storage.append_event(
            _make_event(event_id=f"e{i}", judgment_point_id=f"jp-{i}", project_id="proj-1")
        )
...
@pytest.mark.asyncio
async def test_save_and_get_judgment_point(storage: MemoryStorage) -> None:
    point = _make_point()
...
@pytest.mark.asyncio
async def test_get_judgment_point_returns_none_for_unknown_id(storage: MemoryStorage) -> None:
    result = await storage.get_judgment_point("nonexistent")
    assert result is None


@pytest.mark.asyncio
async def test_get_judgment_points_by_project(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_status(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_category(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_materiality_score_min(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_materiality_score_max(storage: MemoryStorage) -> None:
    await storage.save_judgment_point(_make_point(point_id="jp-1", score=3))
    await storage.save_judgment_point(_make_point(point_id="jp-2", score=7))
    await storage.save_judgment_point(_make_point(point_id="jp-3", score=10))
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_created_after(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_judgment_points_filter_by_created_before(storage: MemoryStorage) -> None:
    await storage.save_judgment_point(_make_point(point_id="jp-1", created_at=EARLIER))
    await storage.save_judgment_point(_make_point(point_id="jp-2", created_at=LATER))
...
@pytest.mark.asyncio
async def test_get_judgment_points_with_pagination(storage: MemoryStorage) -> None:
    for i in range(5):
...
@pytest.mark.asyncio
async def test_save_and_get_policy(storage: MemoryStorage) -> None:
    policy = _make_policy()
...
@pytest.mark.asyncio
async def test_get_policy_returns_none_for_unknown_id(storage: MemoryStorage) -> None:
    result = await storage.get_policy("nonexistent")
    assert result is None


@pytest.mark.asyncio
async def test_get_policies_returns_sorted_by_priority(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_get_policies_filters_by_project(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_update_policy(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_delete_policy(storage: MemoryStorage) -> None:
...
@pytest.mark.asyncio
async def test_delete_policy_noop_for_unknown_id(storage: MemoryStorage) -> None:
    # Should not raise
...
@pytest.mark.asyncio
async def test_clear_removes_all_data(storage: MemoryStorage) -> None:
    await storage.append_event(_make_event())
    await storage.save_judgment_point(_make_point())
    await storage.save_policy(_make_policy())

    storage.clear()

    assert await storage.get_events("jp-1") == []
    assert await storage.get_judgment_point("jp-1") is None
    assert await storage.get_policy("pol-1") is None
