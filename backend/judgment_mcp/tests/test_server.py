...
``MCPServer.read_resource()``, and authority enforcement.

Actor identity is now determined by execution context, not by caller
parameters.  All MCP tool calls are attributed to the agent.
"""
...
def mcp_server(storage):
    return create_judgment_mcp_server(storage, project_id=PROJECT_ID)
...
# ===========================================================================
# Tool invocation tests: judgment_resolve (including authority enforcement)
# ===========================================================================


class TestJudgmentResolve:
    async def test_agent_blocked_human_authority(self, mcp_server, client):
        """MCP tool calls (agent) cannot resolve human-authority points."""
        ...
        data = json.loads(_extract_tool_text(result))
        assert data["error"] == "authority_denied"
        assert "human authority" in data["message"].lower()
        assert data["authority_mode"] == "human"

    async def test_agent_blocked_collaborative_authority(self, mcp_server, client):
        """MCP tool calls (agent) cannot resolve collaborative-authority points."""
    ...
    async def test_agent_can_resolve_delegated_authority(self, mcp_server, client):
        """MCP tool calls (agent) CAN resolve delegated-authority points."""
        candidate = await _create_candidate_via_client(client, authority={"mode": "human"})
        await client.promote(candidate.id, USER_ACTOR)
        # Delegate to the agent
        await client.delegate(candidate.id, "agent:claude", "policy-1", USER_ACTOR)
...
class TestResources:
    async def test_list_judgments_empty(self, mcp_server):
        contents = await mcp_server.read_resource(f"judgment://projects/{PROJECT_ID}/judgments")
    ...
    async def test_get_single_judgment(self, mcp_server, client):
        ...
        contents = await mcp_server.read_resource(
            f"judgment://projects/{PROJECT_ID}/judgments/{candidate.id}"
        )
        items = list(contents)
        data = json.loads(items[0].content)
        assert data["id"] == candidate.id
    ...
    async def test_get_judgment_not_found(self, mcp_server):
        contents = await mcp_server.read_resource(
            f"judgment://projects/{PROJECT_ID}/judgments/nonexistent"
        )
        items = list(contents)
        data = json.loads(items[0].content)
    ...
    async def test_get_judgment_events(self, mcp_server, client):
        ...
        contents = await mcp_server.read_resource(
            f"judgment://projects/{PROJECT_ID}/judgments/{candidate.id}/events"
        )
        items = list(contents)
        data = json.loads(items[0].content)
        assert len(data) == 2
    ...
    async def test_list_policies_empty(self, mcp_server):
        contents = await mcp_server.read_resource(f"judgment://projects/{PROJECT_ID}/policies")
        items = list(contents)
        data = json.loads(items[0].content)
        assert data == []
