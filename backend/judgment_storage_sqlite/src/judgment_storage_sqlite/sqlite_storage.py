...
class SqliteStorage(JudgmentStorage):
    ...
    def __init__(self, url: str = "sqlite+aiosqlite:///judgment.db") -> None:
        ...
        # Register SQLite pragmas for WAL mode and concurrency.
    ...
    async def save_judgment_point(self, point: JudgmentPoint) -> None:
        ...
        decision_key = data.get("decisionKey")
        async with self._session_factory() as session:
            ...
            if existing is not None:
                ...
                existing.decision_key = decision_key  # type: ignore[assignment]
    ...
    async def get_by_decision_key(self, project_id: str, decision_key: str) -> JudgmentPoint | None:
        ...
        stmt = (
            select(JudgmentPointRow)
            .where(JudgmentPointRow.project_id == project_id)
            .where(JudgmentPointRow.decision_key == decision_key)
            .limit(1)
        )
        async with self._session_factory() as session:
            result = await session.execute(stmt)
    ...
    async def set_decision_key(self, point_id: str, decision_key: str) -> None:
        ...
        async with self._session_factory() as session:
            row = await session.get(JudgmentPointRow, point_id)
            if row is not None:
                row.decision_key = decision_key  # type: ignore[assignment]
                await session.commit()
    ...
    async def set_session_id(self, point_id: str, session_id: str) -> None:
        ...
        async with self._session_factory() as session:
            row = await session.get(JudgmentPointRow, point_id)
            if row is not None:
                row.session_id = session_id  # type: ignore[assignment]
                await session.commit()
    ...
    async def set_resolution_scope(self, point_id: str, scope: dict[str, Any]) -> None:
        ...
        async with self._session_factory() as session:
            row = await session.get(JudgmentPointRow, point_id)
            if row is not None:
                row.resolution_scope = scope  # type: ignore[assignment]
                await session.commit()
    ...
    async def get_resolution_scope(self, point_id: str) -> dict[str, Any] | None:
        ...
        if row is None:
            return None
        return row.resolution_scope  # type: ignore[return-value]
    ...
    async def get_session_id(self, point_id: str) -> str | None:
        ...
        if row is None:
            return None
        return row.session_id  # type: ignore[return-value]
    ...
    async def get_points_by_session(self, project_id: str, session_id: str) -> list[JudgmentPoint]:
        ...
        stmt = (
            select(JudgmentPointRow)
            .where(JudgmentPointRow.project_id == project_id)
            .where(JudgmentPointRow.session_id == session_id)
        )
        async with self._session_factory() as session:
            result = await session.execute(stmt)
            rows = result.scalars().all()
        return [JudgmentPoint.model_validate(r.data) for r in rows]
