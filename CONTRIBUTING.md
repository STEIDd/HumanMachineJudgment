...
uv --version      # should print a recent version
...
This is a monorepo managed with pnpm workspaces (TypeScript) and uv workspaces (Python). The workspace roots are defined in `pnpm-workspace.yaml` and `backend/pyproject.toml`.

```
backend/            Python packages (uv workspaces)
  judgment_core/      Core domain logic, state machine, materiality, events
  judgment_sdk/       Developer SDK
  judgment_storage_memory/   In-memory storage adapter
  judgment_storage_sqlite/   SQLite storage adapter
...
packages/           TypeScript packages (pnpm workspaces)
  judgment-schemas/   JSON Schema definitions and TypeScript types
  judgment-ui/        React UI component library (18 components)

apps/               Runnable applications
  review-console/     Judgment review console (served by `hmj serve`)
...
evals/              Evaluation harness and fixtures (Python/pytest)
...
- Use [pytest](https://docs.pytest.org/) for Python unit and integration tests.
- Use [Vitest](https://vitest.dev/) for TypeScript unit and integration tests.
...
- Place Python tests adjacent to the code they test, using the `test_` prefix.
- Place TypeScript tests adjacent to the code they test, using the `.test.ts` or `.spec.ts` suffix.
...
1. Add the tool or resource definition in `backend/judgment_mcp/`.
