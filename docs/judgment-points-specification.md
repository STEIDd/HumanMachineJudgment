...
Three reference applications are planned:

- Reference demonstration (`apps/reference-demo`): an interactive React application showing Judgment Points in a realistic technical workflow.
- Reference server (`apps/reference-server`): an HTTP API server providing endpoints for Judgment Point operations.
...
- `apps/`: Reference applications (reference-server, reference-demo, documentation).
...
The OpenAPI specification has not yet been created. The reference server (`apps/reference-server`) currently provides only a health check endpoint at `GET /health`. The API surface described above represents planned functionality that will be implemented in Phase 2 and Phase 3 of the project roadmap.
...
The MCP server is built using the MCP TypeScript SDK v2.0.0, which uses split packages:

- `@modelcontextprotocol/server` for server implementation and request handling.
- `@modelcontextprotocol/client` for client-side functionality, used primarily in testing and in cross-server communication.
...
The MCP server package (`packages/judgment-mcp`) is in early development. The resource and tool definitions described in this section represent planned functionality. The server's entry point exists and can be initialized, but the full tool and resource implementations are not yet complete. Development of the MCP server is planned for Phase 2 of the project roadmap.
...
The LangGraph adapter package (`packages/judgment-langgraph`) is in the early design phase. The integration patterns described in this section represent planned functionality based on the current LangGraph API surface. Implementation will begin in Phase 2 of the project roadmap and will be validated against both the Python and JS/TS LangGraph runtimes.
