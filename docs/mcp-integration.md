...
| Server SDK   | MCP Python SDK (`mcp` >= 1.0) |
...
The MCP server is launched automatically by Claude Code when HMJ is
connected. The `hmj connect claude` command writes a `.mcp.json` entry:
...
The server uses the project's SQLite storage (discovered via
`.judgment/project.toml`) and runs as `agent:claude` identity. The
agent cannot self-declare its identity.

## Identity and Authority

All MCP tool calls are attributed to `Actor(id="agent:claude",
type="agent")`. The agent identity is fixed at the server level and
cannot be overridden by tool parameters.

When the agent calls `judgment_resolve`, the server checks the
judgment point's authority configuration:

- **human** authority: agent is denied, returns `authority_denied` error
- **collaborative** authority: agent is denied
- **delegated** authority: agent is allowed (point was explicitly
  delegated by a human)
- **rule** authority: agent is allowed if the rule permits it
