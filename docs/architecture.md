...
In production use, HMJ operates as a single `hmj` binary that provides
three integration points converging on the same SQLite database:
...
**Trust levels**: Each access path has a fixed identity. Hooks run as
`system:hook`, the MCP server runs as `agent:claude`, and CLI commands
run as `user:{username}`. Actors never self-declare their identity.

**Storage**: SQLite with WAL mode, `busy_timeout=5000`, and
`synchronous=NORMAL` for safe concurrent access from hooks, MCP, CLI,
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hmj`                     | Operational runtime CLI: project initialization, Claude Code integration (hooks + MCP), review console, and judgment point management commands                                                                                     |
...
| `judgment_storage_memory` | In-memory storage adapter implementing the storage interface (testing)                                                                                                                                                             |
| `judgment_storage_sqlite` | SQLite storage adapter with WAL mode for production concurrent access                                                                                                                                                              |
...
| `reference_server`        | FastAPI-based HTTP reference server (testing/development only)                                                                                                                                                                     |
...
| ---------------- | ---------------------------------------- | ---------------------------------------------------- |
| `review-console` | `@human-machine-judgment/review-console` | Review console web application served by `hmj serve` |
| `documentation`  | `@human-machine-judgment/documentation`  | Static documentation site                            |
...
(core + sdk)  (core)
         |
        hmj
  (core + sdk + mcp + sqlite + Click + FastAPI)
```
